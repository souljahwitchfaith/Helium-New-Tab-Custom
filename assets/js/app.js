...
searchInput.addEventListener('keydown', (e) => {
    if (e.key === 'Backspace') {
        ...
        // Режим "Пролом стенки":
        // Если в строке что-то вроде "!r " (бенг и пробел)
        if (val.match(/^![a-z]+\s$/i)) {
            // Принудительно стираем пробел и последнюю букву бенга
            // чтобы скрипт перестал считать это активной командой
            e.stopPropagation(); // Останавливаем другие скрипты
            searchInput.value = val.substring(0, val.length - 1);
        }
    }
}, true); // true важен — это фаза перехвата (capture)
